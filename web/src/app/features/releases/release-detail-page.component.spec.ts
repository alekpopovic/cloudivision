...
class FakeApiClient {
  releases() { return of([{ name: 'release-1', namespace: 'ci', spec: { projectRef: 'p', environmentRef: 'prod', buildRunRef: 'build-1', image: { repository: 'example/app', tag: 'v1' }, strategy: 'gitops', promotionMode: 'pull-request' }, status: { phase: 'GitOpsChangeCommitted', gitCommit: 'abc123', deployment: { provider: 'argocd', syncStatus: 'OutOfSync', healthStatus: 'Progressing', observedRevision: 'main@abc123', observedAt: '2026-08-23T00:00:00Z' }, pullRequest: { url: 'https://github.com/acme/gitops/pull/4' }, conditions: [] } }]); }
