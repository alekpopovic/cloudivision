...
Status is `Ready` or `Error` with conditions and observed generation. The Job executor currently executes commands inside the runner image; per-step image isolation is not yet implemented and is documented as a dogfood limitation.
